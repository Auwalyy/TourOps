...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-white text-neutral-900 font-sans">
      ...
      <header className="sticky top-0 z-50 border-b border-neutral-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-3 px-4 py-4 sm:px-6">
          ...
          <nav className="hidden items-center gap-8 text-sm text-neutral-600 md:flex">
            ...
            <a href="#roles" className="hover:text-neutral-900 transition-colors">Who It&apos;s For</a>
          </nav>
          <div className="flex items-center gap-2 sm:gap-3">
            <Link href="/login" className="rounded-lg px-3 py-2 text-sm font-medium text-neutral-700 hover:bg-neutral-100 transition-colors">
              Log in
            </Link>
      ...
      {/* Hero */}
      <section className="border-b border-neutral-200 bg-white pt-20">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <h1 className="text-4xl font-semibold leading-[1.1] tracking-tight text-neutral-900 sm:text-5xl">
          ...
          <div className="mt-9 flex flex-col items-center justify-center gap-3 sm:flex-row">
            ...
            <Link href="/login" className="inline-flex w-full items-center justify-center rounded-lg border border-neutral-300 px-6 py-3 text-sm font-medium text-neutral-700 hover:bg-neutral-50 transition-colors sm:w-auto">
              Log in
            </Link>
          </div>
        </div>
        ...
        <div className="mx-auto mt-16 max-w-4xl px-4 sm:px-6">
          <div className="overflow-hidden rounded-t-xl border border-neutral-200 bg-white">
            {/* browser bar */}
            <div className="flex items-center gap-2 border-b border-neutral-200 bg-neutral-50 px-4 py-3">
              <span className="h-2.5 w-2.5 rounded-full bg-neutral-300" />
              <span className="h-2.5 w-2.5 rounded-full bg-neutral-300" />
              <span className="h-2.5 w-2.5 rounded-full bg-neutral-300" />
              <div className="ml-4 flex-1 rounded-md bg-neutral-100 px-3 py-1 text-xs text-neutral-400">app.tourops.com/dashboard</div>
            </div>
            {/* mock content */}
            <div className="grid grid-cols-1 gap-4 bg-neutral-50 p-4 sm:grid-cols-2 sm:p-6 lg:grid-cols-4">
              {[
                ...
                <div key={k.label} className="rounded-lg border border-neutral-200 bg-white p-4">
                  <div className="text-xl font-semibold text-neutral-900">{k.val}</div>
                  <div className="mt-1 text-xs text-neutral-400">{k.label}</div>
                </div>
              ))}
            </div>
            <div className="grid grid-cols-1 gap-4 bg-neutral-50 px-4 pb-4 sm:px-6 sm:pb-6 lg:grid-cols-3">
              <div className="rounded-lg border border-neutral-200 bg-white p-4 lg:col-span-2">
                <div className="mb-3 text-xs font-medium uppercase tracking-wide text-neutral-400">Revenue Overview</div>
              ...
              <div className="rounded-lg border border-neutral-200 bg-white p-4">
                <div className="mb-3 text-xs font-medium uppercase tracking-wide text-neutral-400">Recent Activity</div>
                <div className="space-y-2">
                  {['Booking KAN-BKG-26-000142', 'Visa approved — Aisha B.', '₦450,000 payment verified'].map((a) => (
                    <div key={a} className="flex items-center gap-2">
                      ...
                      <span className="truncate text-xs text-neutral-500">{a}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* How it works */}
      <section id="how" className="border-y border-neutral-200 bg-neutral-50 py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="mb-12 max-w-2xl">
            <h2 className="text-2xl font-semibold tracking-tight text-neutral-900 sm:text-3xl">Up and running quickly</h2>
            <p className="mt-3 text-neutral-600">No lengthy setup, no training course.</p>
          </div>
          <div className="grid gap-10 sm:grid-cols-3">
            {steps.map(({ step, title, desc }) => (
              <div key={step}>
                <div className="text-sm font-semibold text-blue-600">{step}</div>
                <h3 className="mt-2 font-medium text-neutral-900">{title}</h3>
                <p className="mt-1.5 text-sm leading-relaxed text-neutral-600">{desc}</p>
              </div>
            ))}
          </div>
      ...
      <section id="roles" className="bg-white py-20">
        <div className="mx-auto max-w-5xl px-6">
          <div className="mb-12 max-w-2xl">
            <h2 className="text-2xl font-semibold tracking-tight text-neutral-900 sm:text-3xl">A role for everyone</h2>
            <p className="mt-3 text-neutral-600">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {roles.map(({ icon: Icon, label, desc }) => (
              <div key={label} className="flex items-start gap-3 rounded-xl border border-neutral-200 p-5">
                <Icon size={18} strokeWidth={1.75} className="mt-0.5 shrink-0 text-neutral-400" />
                <div>
      ...
      {/* CTA */}
      <section className="border-t border-neutral-200 py-20" style={{ backgroundColor: 'var(--color-navy)' }}>
        <div className="mx-auto max-w-2xl px-6 text-center">
          <h2 className="text-2xl font-semibold tracking-tight text-white sm:text-3xl">
            Ready to get your operation in order?
          </h2>
          <p className="mt-3 text-neutral-400">Create an account and set up your agency in a few minutes.</p>
          <div className="mt-8 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <Link href="/register" className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-white px-6 py-3 text-sm font-medium text-neutral-900 hover:bg-neutral-100 transition-colors sm:w-auto">
              Create your account <ArrowRight size={16} />
            </Link>
